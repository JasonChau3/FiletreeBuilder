 ...
 * Function prototype: int timeCompare (cons void * p1, const void * p2);
...
if:
    ...
    ldr     r0, [fp, P1_OFFSET]         
    ldr     r3, =offset_time            @load in the time offset
    ldr     r3, [r3]
    ldr     r0, [r0, r3]                @ get 1st time into r0
    ldr     r1, [fp, P2_OFFSET]
    ldr     r1, [r1, r3]                @ get 2nd time into r1
    cmp     r0, r1
    bge     else_if                     @compare r0, r1
                                        @branch out if its greater or equal
                                        @ return -1
    mov     r0, -1
    b       end_if
...
else_if:
    ...
    ldr     r3, =offset_time 
    ldr     r3, [r3]                    @load in the time offset
    ldr     r0, [fp, P1_OFFSET]
    ldr     r0, [r0, r3]                @ get 1st time into r0
    ldr     r1, [fp, P2_OFFSET]
    ldr     r1, [r1,r3]                 @ get 2nd name into r1
    cmp     r0, r1
    beq     else
                                        @return 1 becuase modification time
    mov     r0, 1                       @is older
    b       end_if
